...
public class HastyEnemy : EnemyBase
{
    /// <summary>
    ...
    [Tooltip("The player check component used to test whether this enemy see a player to the left.")]
    public PlayerCheck playerTestLeft;
    [Tooltip("The player check component used to test whether this enemy see a player to the right.")]
    public PlayerCheck playerTestRight;
    ...
    [Tooltip("How much faster this enemy moves when haste")]
            ...
            case WalkDirections.Left:
                ...
                if (SeePlayer())
                {
                    if (TestWall() || GetIsNearEdge())
                        ...
                        return Vector3.left * (moveSpeed + moveSpeedBonus) * Time.deltaTime;
                }
                else
                {
            ...
            case WalkDirections.Right:
                ...
                if (SeePlayer())
                {
                    if (TestWall() || GetIsNearEdge())
                        ...
                        return Vector3.right * (moveSpeed + moveSpeedBonus) * Time.deltaTime;
                }
                else
                {
    ...
    {
        if ((TestWall() && !SeePlayer()) || (GetIsNearEdge() && !SeePlayer()) || HearPlayer())
        {
            TurnAround();
        }
    ...
    protected virtual bool SeePlayer()
    {
        ...
        {
            case WalkDirections.Left:
                if (playerTestLeft != null)
                {
                    return playerTestLeft.CheckPlayer();
                }
                break;
            case WalkDirections.Right:
                if (playerTestRight != null)
                {
                    return playerTestRight.CheckPlayer();
                }
                break;
        }
        return false;
    ...
    protected virtual bool HearPlayer()
    {
        switch (walkDirection)
        {
            case WalkDirections.Right:
